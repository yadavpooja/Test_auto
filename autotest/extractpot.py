...
class ExtractPot:
    ...
    def extractPot(self):
        ...
        rpm2cpio = subprocess.Popen(command, stdout=subprocess.PIPE, cwd=self.srpm_dir)
        command = "cpio -idm"
        command = shlex.split(command)
        subprocess.call(command, stdin=rpm2cpio.stdout, cwd=self.srpm_dir)

        command = "tar -xf "+tar_name+".tar.xz"
        command = shlex.split(command)
        subprocess.call(command, cwd=self.srpm_dir)
        log.info("src.rpm unpacked in %s", self.srpm_dir)

        po_dir = self.srpm_dir+"/"+tar_name+"/po"

        command = "intltool-update -p"
        command = shlex.split(command)
        subprocess.call(command, cwd=po_dir)
        log.info("TODO insert command output here.")

        pot_name = self.program_name+".pot"

        os.rename(po_dir+'/'+pot_name, self.home+"/.autotest/pot_files/"+pot_name)
    ...
    def getStats(self):
        ...
        command = shlex.split(command)
        stats = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=path)
